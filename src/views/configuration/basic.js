...
const BasicConfigPage = () => {
  ...
  const schema = Joi.object({
    reminderTime: Joi.number().optional().label(t("reminder_time")),
  });
  ...
  const handleSubmit = async () => {
    ...
    if (formData.reminderTime !== initialValues.reminderTime) {
      const updatedData = _.pick(formData, ["reminderTime"]);
      console.log("Updated data for reminderTime:", updatedData);
      ...
      try {
        await configServices.updateConfig(updatedData);
        setInitialValues((prevValues) => ({ ...prevValues, ...updatedData }));
        toast.success(t("common_success"));
      } catch (error) {
  ...
  return (
    <div className="shadow sm:rounded-lg bg-white p-4 mt-2 mb-5 row g-3">
      <div className="grid justify-end">
        <CFormSwitch
          label={t("enable_update_mode")}
          id="formSwitchCheckDefault"
          onChange={() => {
            setUpdateMode(!updateMode);
            setFormData(initialValues); // Reset form data on toggle
          }}
        />
      </div>
